...
from typing import List, Tuple, TypeAlias, Self
import logging
...
RotationMatrix: TypeAlias = np.ndarray
...
def rotation_matrix_to_roll_tilt_pan(R_mat: np.ndarray) -> tuple[float, float, float]:
    # Convert using ZYX convention = roll (around Z), tilt (Y), pan (X)
...
def convert_intrinsics_to_calibration_matrix(
    sensor_wh: Vector2D, focal_length_xy: Vector2D
) -> CalibrationMatrix:
...
def convert_rvec_tvec_to_camera_pose(
    ...
    """
    rotation_matrix: RotationMatrix3D = cv.Rodrigues(np.asarray(rvec))[0]  # type: ignore[assignment]
...
) -> tuple[RotationMatrix, Vector3D, DistortionCoefficients]:
...
class Camera:
    ...
    def __init__(self, sensor_wh: tuple[int, int] | Vector2D, camera_params: CameraParams) -> None:
        self._sensor_wh = np.asarray(sensor_wh)
    ...
    def from_keypoint_correspondences(
        ...
        homography_matrix = calculate_homography(world_points, image_points, alpha=1.0)
        success, focal_length_xy = calculate_focal_length_from_homography(
            homography_matrix, sensor_wh
        )
        if not success:
            msg = "Failed to estimate focal length automatically."
            raise ValueError(msg)

        calibration_matrix = convert_intrinsics_to_calibration_matrix(
            sensor_wh, focal_length_xy
        )
