...
class CameraParams:
    ...
    def __init__(self, camera_pose: CameraPose, camera_distortion: CameraDistortion, focal_length: float):
        self.camera_pose = camera_pose
        self.camera_distorion = camera_distortion
    ...
    def to_array(self) -> np.ndarray:
        ...
        """
        return np.concatenate((self.camera_pose.to_array(), self.camera_distorion.to_array(), np.array([self.focal_length])), axis=0)
    ...
    def __eq__(self, other: Any, tol: float = 1e-6):
        ...
        if isinstance(self, other.__class__):
            return (self.camera_pose == other.camera_pose) and (self.camera_distorion == other.camera_distortion) and ((self.focal_length - other.focal_length) < tol)
        return False

    def __repr__(self):
        return f"CameraParams(camera_pose=CameraPose({str(self.camera_pose)}, camera_distorion=CameraDistortion({str(self.camera_distorion)}),focal_length={self.focal_length})"
