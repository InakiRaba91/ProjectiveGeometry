...
def get_gravity_acc(sim_config: PhysicsConfig) -> np.ndarray:
    return np.array([0, 0, -sim_config.gravity_yds_per_sec2])


def get_friction_acc(vel, sim_config: PhysicsConfig) -> np.ndarray:
    v_xy = vel[:2]
...
def get_drag_acc(vel, sim_config: PhysicsConfig) -> np.ndarray:
    # F = 0.5 * rho * v^2 * Cd * A
    # a = F/m
...
def get_magnus_acc(vel, omega, sim_config: PhysicsConfig) -> np.ndarray:
