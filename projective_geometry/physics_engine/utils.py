...
def draw_trajectory_comparison(
    ...
    cv2.imwrite((output_dir / f"{output_prefix}_observations.png").as_posix(), np.concatenate([image_broadcast_obs, image_birdseye_obs], axis=1))
    cv2.imwrite((output_dir / f"{output_prefix}_estimated.png").as_posix(), np.concatenate([image_broadcast, image_birdseye], axis=1))