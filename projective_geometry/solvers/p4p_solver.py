...
def p4p(
    ...
    camera_pose_candidates = solver(
        pts_world=pts_world[:, sampled_indices[:-1]],  # type: ignore
        pts_img_cam=pts_img_cam[:, sampled_indices[:-1]],  # type: ignore
    )
    ...
    for i, camera_pose in enumerate(camera_pose_candidates):
        cost = reproj_error_points_pose(
            camera_pose=camera_pose, pts_world=pts_world[:, [pt_idx]], pts_cam=pts_img_cam[:, [pt_idx]]
        )
        if cost < min_cost:
